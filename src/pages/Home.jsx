...
const Home = () => {
        ...
        const fetchVideosByRegion = async (region, setVideos) => {
            try {
                const response = await fetch(
                    `https://youtube.googleapis.com/youtube/v3/search?part=snippet&type=video&q=${region}+history&order=date&maxResults=10&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`
                );
    ...
    return (
        <Main title="각 대륙별 역사를 다루는 사이트" description="각 대륙별 역사를 다루는 사이트입니다.">
